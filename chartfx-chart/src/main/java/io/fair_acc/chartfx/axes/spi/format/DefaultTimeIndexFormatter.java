 ...
 * @author rstein
 */
public class DefaultTimeIndexFormatter extends AbstractFormatter {
     ...
     */
    public DefaultTimeIndexFormatter() {
        this(null);
     ...
     */
    public DefaultTimeIndexFormatter(final Axis axis) {
        super(axis);
        setTickUnitSupplier(DefaultTimeIndexFormatter.DEFAULT_TICK_UNIT_SUPPLIER);
        ...
        for (int i = 0; i < dateFormat.length; i++) {
            ...
            if (format.contains(DefaultTimeTickUnitSupplier.HIGHRES_MODE)) {
                dateFormat[i] = DefaultTimeIndexFormatter.HIGHRES_FORMATTER;
            } else {
    ...
    public String formatHighResString(final Number utcValueSeconds) {
        ...
        final LocalDateTime dateTime = LocalDateTime.ofEpochSecond(longUTCSeconds, longNanoSeconds,
                getTimeZoneOffset());
        return dateTime.format(DefaultTimeIndexFormatter.HIGHRES_FORMATTER).concat(Long.toString(timeUS % 1000)).concat("us")
